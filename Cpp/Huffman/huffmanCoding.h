...
void HuffmanCoding::createCodewords(HuffmanNode *p, unsigned long codeword, int level) {
    ...
    p->codewordLen = level;                 // and its length,
    }
...
void HuffmanCoding::encode(ifstream& fIn) {
    ...
    for (fIn.get(ch); !fIn.eof(); ) {
        ...
        if (p == 0)error("A problem in encode()");
        if (p->codewordLen < maxPack - packCnt) {    // if enough room in 
            pack = (pack << p->codewordLen) | p->codeword; // pack to  // store new
            packCnt += p->codewordLen;              // codeword, shift 
        }                                            // its content to the // left and attach  // new codeword;
        else {                                       // otherwise move
            ...
            if (bitsLeft != p->codewordLen) {       // number of left
                ...
                hold >>= p->codewordLen - bitsLeft; // codeword is  // longer than room 
                pack |= hold;                      // left, transfer
            }                                       // only as many bits as// can fit in pack;
            else pack |= p->codeword;               // if new codeword// exactly fits in// pack, transfer it;
            output(pack);                           // output pack as// four chars;
            if (bitsLeft != p->codewordLen) {       // transfer
                pack = p->codeword;                // unprocessed bits
                packCnt = maxPack - (p->codewordLen - bitsLeft); // of 
                packCnt = p->codewordLen - bitsLeft; // new codeword to  // pack;
            }else packCnt = 0;
        }
        ch = ch2;
    }
    if (packCnt != 0) {
        pack <<= maxPack - packCnt; // transfer leftover codewords and  // some 0s
        output(pack);
    } 
} 
